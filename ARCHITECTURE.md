...
**Bug trouvé par l'utilisateur** : téléphone connecté au SSID `OXY-LD2` mais page inaccessible, alors qu'un PC y arrivait sans souci sur le même réseau. Cause : ce point d'accès n'a pas d'Internet réel (AP local pur) — les téléphones (Android/iOS) et Windows interceptent le trafic web pour vérifier la connectivité (endpoints `generate_204`, `hotspot-detect.html`, `connecttest.txt`, `ncsi.txt`...) et bloquent la navigation normale tant que ces vérifications échouent, contrairement à un PC généralement plus permissif. OXY-LD a déjà exactement ces routes de contournement (même symptôme sur le même type de point d'accès) — reprises telles quelles, plus `onNotFound` qui redirige vers `/materiel` au lieu de 404. **Non re-testé sur téléphone après correction** (le PC continue de fonctionner, confirmé après ce changement).
