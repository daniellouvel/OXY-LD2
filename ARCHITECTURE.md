...
**Bug trouvé par l'utilisateur, en deux temps** : téléphone connecté au SSID `OXY-LD2` mais page inaccessible, alors qu'un PC y arrivait sans souci sur le même réseau.
1. Première hypothèse (insuffisante) : ce point d'accès n'a pas d'Internet réel (AP local pur) — ajout des routes HTTP de contournement de la vérification de connectivité (`generate_204`, `hotspot-detect.html`, `connecttest.txt`, `ncsi.txt`) qu'OXY-LD utilise déjà pour le même symptôme. **N'a pas suffi** ("tj rien sur mon tel").
2. **Cause réelle** : la vérification de connectivité résout d'abord un nom de domaine (ex. `connectivitycheck.gstatic.com`) par DNS *avant* de faire la requête HTTP. Sans serveur DNS sur cet AP, la résolution échoue et les routes HTTP ci-dessus ne sont jamais atteintes. OXY-LD lance justement un `DNSServer` qui répond à **tous** les noms de domaine avec sa propre IP (`dnsServer.start(53, "*", ...)`) — repris à l'identique (`dns_server.start(53, "*", WiFi.softAPIP())`), avec `dns_server.processNextRequest()` appelé en tout début de `loop()`, avant tout retour anticipé (le DNS doit continuer à répondre même quand la mesure ADS1115 échoue).

Un PC n'a pas ce problème car son système d'exploitation ne fait généralement pas ce type de vérification bloquante de connectivité avant d'autoriser la navigation manuelle.
