...
- `web_ui/` — génération pure du HTML/JSON du serveur de configuration (pages matériel/plongée/tables), `std::string`, aucune dépendance WiFi. Design refondu (cartes, pastilles de statut, palette cohérente). Testé en `native` (76 tests). Le câblage réel (`WiFi.softAP`, routes `AsyncWebServer`, mutex) vit dans `src/main.cpp` — inclut désormais `POST /rest` pour régler le temps de repos avant auto-calibration (persisté en flash).
